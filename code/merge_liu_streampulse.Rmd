...
Christa Torrens cleaned and streamlined the script but did not run it: this was a mistake; quite a bit of information seems to be missing, as well as several steps, including saving updated csvs. Other CT amendments include adding this information, R packages, comments, and changing the read-data paths match the current folder structure by using the 'here' package. 
...
UPDATE - Matt N. never did share that info, and the COMIDs are wrong. All of Liu's COMIDs have 8 digits, and run from 110000001 to 86007275. 114 our of 222 of Matt's merged COMID's have < 8 digits; and it's not simply due to dropping a terminal '0' or 4, because just adding in 0s to = 8 digits puts several of the COMIDs over 86007275. This is an obvious mismatch (perhaps a match to some other column?) and it's unclear where those values are from/ how they were assigned; therefore I am not confident that the entries that *do* have 8 digits are actually matched to the Liu et al. COMIDs.
...
liu_co2 <- read.csv(here("data/aquatic/streams/Liu_data_DL_v20210902/co2f.csv"), header = TRUE, sep = ",")
...
k <- read.csv(here("data/aquatic/streams/Liu_data_DL_v20210902/k.csv"), header = TRUE, sep = ",")  ## PATH? 
