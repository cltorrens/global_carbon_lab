...
monthly_avg_df <- df %>%
  ...
  summarize(
   ...
   mutate(days = days_in_month[month], 
          GPP_monthly_raw = avgday_GPP * days,
          GPP_monthly.lower = avgday_GPP.lower * days,
          GPP_monthly.upper = avgday_GPP.upper * days,
          ER_monthly_raw = avgday_ER * days,
          ER_monthly.lower = avgday_ER.lower * days,
          ER_monthly.upper = avgday_ER.upper * days,
          ) %>%
  group_by(site_name) %>%
  mutate(
    frac_GPP = GPP_monthly_raw / sum(GPP_monthly_raw, na.rm = TRUE),
    frac_ER  = ER_monthly_raw  / sum(ER_monthly_raw,  na.rm = TRUE)
  ) %>%
  ungroup() %>%
  ...
  mutate(
    GPP_monthly_scaled = frac_GPP * ann_GPP,
    ER_monthly_scaled  = frac_ER  * ann_ER,
